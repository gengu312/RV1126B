...
主页会优先提示新手先完成 IO / ADC 实验。各功能页底部固定提供“实验台主页”和“返回系统桌面”两个按钮；较长页面提供独立滚动区域，不会再把返回入口挤出屏幕。
...
安装位置为 `/opt/ui/src/apps/rv1126blab`。实验台入口固定为第二页 `apk2.cfg` 的第一个图标，不会占用底部 Dock 的 `apk3.cfg`，也不会落入第二页清理模块覆盖的第二行。如果板上已安装独立的 `adcled`，安装器会把“ADC 调速灯”恢复为第一页第 13 个图标，即现有 12 个图标下方的第一个空白格；该位置不会被清理模块或 Dock 遮挡。安装脚本不会自动重启开发板，重启后桌面会出现两个入口。

程序顶部和底部都提供返回入口。由于厂商的控制中心/动态岛可能覆盖屏幕顶部触摸区，底部红色“返回系统桌面”是主要退出入口；点击后程序会立即隐藏并退出，随后由 SystemUI 恢复桌面。底部蓝色“实验台主页”只返回本程序主页，不会退出。
